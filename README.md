...
- This is the queue on the opposite side of the pedestrian crossing/intersection. 
- When passing through the intersection, vehicles either go straight or turn right.
- People and vehicles cross or pass through the intersection one by one, reducing the size of the corresponding queue by 1.
...
- Sending an event means placing the container into the event queue for the traffic light.
- Each traffic light has its own queue.
- Traffic lights process events in parallel, independently of each other.
- At the same time, each traffic light processes its events sequentially, in the order they are placed in the queue.
- A traffic light can obtain information about the current state of any other traffic light synchronously (not via an event).

	*Task:
		 devise and describe an adaptive algorithm for the operation of traffic lights
		 to optimize the overall throughput of the intersection
		 depending on the situation at the intersection.
